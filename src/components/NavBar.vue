...
<template>
  <header class="border-border/75 bg-background/75 sticky top-0 z-40 border-b backdrop-blur">
    <nav
      class="flex w-full items-center gap-2 p-3 sm:gap-4 sm:px-8 sm:py-4"
    ...
    >
      <div class="flex shrink-0 items-center gap-2 sm:gap-3">
        ...
        >
          ...
          <span class="font-display text-header-sm hidden tracking-tight sm:inline">{{
            t('common.app.name')
          }}</span>
        </RouterLink>
      ...
      <ul
        class="my-0 flex min-w-0 flex-1 list-none items-center justify-center gap-1 p-0 sm:gap-10 lg:justify-start lg:pl-20"
        role="list"
      >
        <li v-for="element in elements" :key="element.to">
          <RouterLink
            :to="element.to"
            class="text-muted hover:bg-muted/50 hover:text-foreground focus-visible:ring-brand/70 text-md inline-flex items-center gap-2 rounded-lg px-2 py-2 font-semibold transition-colors focus-visible:ring-2 focus-visible:outline-none sm:px-4"
